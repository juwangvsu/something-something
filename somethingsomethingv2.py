...
def parse_args():
    # three _root directory assumed to be under current folder. wang hack
    parser = argparse.ArgumentParser(description='prepare something-something-v2 dataset')
    parser.add_argument('--video_root', type=str, default='20bn-something-something-v2')
    parser.add_argument('--frame_root', type=str, default='20bn-something-something-v2-frames')
    parser.add_argument('--anno_root', type=str, default='annotations')
    parser.add_argument('--num_threads', type=int, default=10000)
    #100 threads too many and cause problem
...
def extract(video, tmpl='%06d.jpg'):
    cmd = 'ffmpeg -i \"{}/{}\" -threads 1 -vf scale=-1:256 -q:v 0 \"{}/{}/%04d.jpg\"'.format(args.video_root, video, args.frame_root, video[:-5])
    os.system(cmd)

def target(video_list):
    for video in video_list:
        if not os.path.isdir(os.path.join(args.frame_root, video[:-5])):
            os.makedirs(os.path.join(args.frame_root, video[:-5]))
        if os.listdir(os.path.join(args.frame_root, video[:-5])) == []:
            print(video)
            #if video=='101971.webm':
            #    print(video)
            #    extract(video)
            extract(video)
...
def decode_video(args):
    ...
    threads = []
    print("number of splits and threads: ", len(splits))
